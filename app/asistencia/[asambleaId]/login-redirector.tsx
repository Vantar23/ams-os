...
function pathFor(tipo: Tipo, accessToken: string) {
  return tipo === "acomodador"
    ? `/acomodador/${accessToken}`
    : `/hermana-apoyo/${accessToken}`
}
...
export function LoginRedirector({
  ...
  React.useEffect(() => {
    ...
    lookupPersonal({
      ...
      window.location.href = pathFor(res.result.tipo, res.result.accessToken)
    })
    return () => {
      cancelled = true
    }
    ...
    <main className="flex min-h-svh items-center justify-center px-5 py-10 sm:py-12">
      <div className="w-full max-w-md">
        ...
        <p className="mt-4 text-[15px] text-muted-foreground sm:text-base">
          Confirma tu teléfono para entrar a tu acceso personal.
        </p>
